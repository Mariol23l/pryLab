<?php
include '../modelo/proveedor.php';

$proveedor = new proveedor();

if ($_POST['funcion'] == 'crear') {
    $nombre = $_POST['nombre'];
    $telefono = $_POST['telefono'];
    $correo = $_POST['correo'];
    $direccion = $_POST['direccion'];
    $razsocial = $_POST['razsocial'];
    $avatar = 'prov_default.png';
    $proveedor->crear($nombre, $razsocial, $telefono, $correo, $direccion, $avatar);
}

if ($_POST['funcion'] == 'editar') {
    $id = $_POST['id'];
    $nombre = $_POST['nombre'];
    $telefono = $_POST['telefono'];
    $correo = $_POST['correo'];
    $direccion = $_POST['direccion'];
    $razsocial = $_POST['razsocial'];
    $proveedor->editar($id,$nombre, $razsocial, $telefono, $correo, $direccion);
}

if ($_POST['funcion'] == 'buscar') {
    $proveedor->buscar();
    $json = array();
    foreach ($proveedor->objetos as $objeto) {
        $json[] = array(
            'id' => $objeto->id_proveedor,
            'nombre' => $objeto->nombre,
            'razsocial' => $objeto->razsocial,
            'telefono' => $objeto->telefono,
            'correo' => $objeto->correo,
            'direccion' => $objeto->direccion,
            'avatar' => '../img/prov/' . $objeto->avatar,
        );
    }
    $jsonstring = json_encode($json);
    echo $jsonstring;
}

if ($_POST['funcion']=='cambiar_logo') {
    $id=$_POST['id_edit_prov'];
    $avatar=$_POST['avatar'];
        if (($_FILES['foto']['type'] == 'image/jpeg') || ($_FILES['foto']['type'] == 'image/png') || ($_FILES['foto']['type'] == 'image/gif')) {
            //Las imagenes se envia por FILES
            $nombre = uniqid() . '-' . $_FILES['foto']['name'];
            $ruta = '../img/prov/' . $nombre;
            move_uploaded_file($_FILES['foto']['tmp_name'], $ruta);
            $proveedor->cambiar_avatar($id, $nombre);
                if ($avatar!='../img/prov/prov_default.png' || $avatar==null) {
                    unlink($avatar);
                }                
            $json = array();
            $json[] = array(
                'ruta' => $ruta,
                'alert' => 'edit'
            );
            $jsonstring = json_encode($json[0]);
            echo $jsonstring;
        } else {
            $json = array();
            $json[] = array(
                'alert' => 'noedit'
            );
            $jsonstring = json_encode($json[0]);
            echo $jsonstring;
        }
    }

    if ($_POST['funcion']=='borrar') {
        $id=$_POST['id'];
        $proveedor->borrar($id);
    }

    if ($_POST['funcion']=='rellenar_proveedores') {
        $proveedor->rellenar_proveedores();
        $json= array();
        foreach($proveedor->objetos as $objeto){
            $json[]=array(
            'id'=>$objeto->id_proveedor,
            'razsocial'=>$objeto->razsocial,
            );
        }
        $jsonstring=json_encode($json);
        echo $jsonstring;
    }

    ?>
