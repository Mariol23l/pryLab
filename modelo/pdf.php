...
function getHtml($id_venta){
    ...
    <header class="clearfix">
      ...
      <div id="project">
        ...
        <div><span>Ruc: </span>'.$objeto->ruc.'</div>
        <div><span>Razon Social: </span>'.$objeto->razsocial.'</div>
        <div><span>Fecha y Hora: </span>'.$objeto->fecha.'</div>
        <div><span>Total: </span>'.$objeto->total.'</div>
      </div>';
      }
    $plantilla.='
    </header>
    <main>
      <table>
        ...
        foreach ($venta_producto->objetos as $objeto) {
          ...
          $plantilla.='<tr>
            .$ob
            <td class="primario">'.$objeto->nombre.'Lote:'.$objeto->lote.' Vcto:'.$objeto->vencimiento.'</td>           
