$(document).ready(function () {
    ...
    function rellenar_proveedores() {
        ...
        $.post('../controlador/ProveedorController.php', { funcion }, (response) => {
            ...
            proveedores.forEach(proveedor => {
                template += `
                    <option value="${proveedor.id}">${proveedor.razsocial}</option>
                `;
            });
            ...
            <div prodId="${producto.id}" prodNombre="${producto.nombre}" prodPrecio="${producto.precio}" prodConcentracion="${producto.concentracion}" prodAdicional="${producto.adicional}" prodLaboratorio="${producto.laboratorio_id}" prodTipo="${producto.tipo_id}" prodPresentacion="${producto.presentacion_id}" prodAvatar="${producto.avatar}" class="col-12 col-sm-6 col-md-4 d-flex align-items-stretch flex-column">
              <div class="card bg-light d-flex flex-fill">
                ...
                <div class="card-body pt-0">
                  <div class="row">
                    <div class="col-7">
                      <h2 class="lead"><b>Precio: ${producto.nombre}</b></h2>
                      <h4 class="lead"><b><i class="fa-solid fa-dollar-sign"></i>${producto.precio}</b></h4>
                      <ul class="ml-4 mb-0 fa-ul text-muted">
                      <li class="small"><span class="fa-li"><i class="fa-solid fa-pills"></i></span>Presentacion: ${producto.presentacion}</li>
                      <li class="small"><span class="fa-li"><i class="fa-solid fa-copyright"></i></span>Tipo: ${producto.tipo}</li>
                      <li class="small"><span class="fa-li"><i class="fas fa-solid fa-flask"></i></span>Concentracion: ${producto.concentracion}</li>
                      <li class="small"><span class="fa-li"><i class="fa-solid fa-prescription-bottle-alt"></i></span>Adicional: ${producto.adicional}</li>
                      <li class="small"><span class="fa-li"><i class="fa-solid fa-flask"></i></span>Laboratorio: ${producto.laboratorio}</li>
                      <li class="small"><span class="fa-li"><i class="fa-solid fa-copyright"></i></span>Tipo: ${producto.tipo}</li>
                      <li class="small"><span class="fa-li"><i class="fa-solid fa-pills"></i></span>Presentacion: ${producto.presentacion}</li>
                      </ul>
                    </div>
                ...
                <div class="card-footer">
                  <div class="text-right">
                    <button class="avatar btn btn-sm bg-teal" type="button" data-toggle="modal" data-target="#cambiologo" title="Editar Avatar del Producto">
                      <i class="fas fa-image"></i>
                    </button>
                    <button class="editar btn btn-sm btn-success" type="button" data-toggle="modal" data-target="#crearproducto" title="Editar Datos del Lote">
                      <i class="fas fa-pencil-alt"></i>
                      </button>
                      <button class="lote btn btn-sm btn-primary float-left" type="button" data-toggle="modal" data-target="#crearlote" title="Crear Nuevo Lote">
    ...
    $('#form-crear-lote').submit(e => {
        ...
        let codlote = $('#codlote').val();
        funcion = 'crear'
        $.post('../controlador/LoteController.php', { funcion, id_producto, proveedor, stock, vencimiento, codlote }, (response) => {
            if (response.includes('noaddlote')) {
                $('#noadd_lote').hide('slow');
                $('#noadd_lote').show(1000);
                $('#noadd_lote').hide(2000);
